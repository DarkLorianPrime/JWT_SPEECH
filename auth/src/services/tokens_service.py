...
from repositories.tokens_repository import TokensRepository, get_tokens_repository
from schemes.users import RefreshRequestSchema, RefreshTokenSchema
from services.redis_service import TokensRedisController, get_tokens_controller
...
class TokensService:
    def __init__(
        self,
        token_controller: Annotated[
            TokensRedisController, Depends(get_tokens_controller)
        ],
    ...
    async def disable_refresh(
        self, payload: dict[str, Any], disable_other: bool
    ) -> None:
        where = [
    ...
    async def save_refresh_token(
        self, refresh_payload: dict[str, Any], user_id: str | UUID
    ):
        token_history = RefreshTokenSchema(
            token_id=refresh_payload["jti"], user_id=user_id
        )
    ...
    async def get_by_jwt_id(self, jti: str | UUID) -> str | Row:
        token_record = await self.token_repository.get_by_query(
