...
from repositories.user_car_repositories import UserCarRepository
from repositories.user_car_repositories import get_user_car_repository
...
class UserCarService:
    def __init__(
        self,
        user_car_repository: Annotated[UserCarRepository, Depends(get_user_car_repository)],
    ):
    ...
    async def get_cars_from_user(self, user_id: UUID):
        return await self.user_car_repository.get_by_query(
            and_(CarUser.user_id == user_id, CarUser.active.is_(True)),
