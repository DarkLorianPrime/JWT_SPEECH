...
class BaseRepository(abc.ABC, Generic[Model]):
    ...
    async def find(self, row_id: str | uuid.UUID | int, model: bool = False) -> Model | None:
        stmt = select(self.model).where(self.model.id == row_id).limit(1)
    ...
    async def get_by_query(
        ...
        stmt = select(*custom_return if custom_return is not None else [self.model]).where(
            query
        )
        ...
        if order_by is not None:
            stmt = stmt.order_by(
                *([order_by] if not isinstance(order_by, Iterable) else order_by)
            )
